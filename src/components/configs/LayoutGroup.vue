...
<template>
  <Group title="Layout" class="layout-group" :default-collapsed="true">
    <div class="item">
      <Tabs :data="{ 'row': 'Row', 'column': 'Column' }" :active-key="layout.direction" @change="key => layout.direction = key"></Tabs>
    </div>
    <div class="item">
      <div class="label">Main Align</div>
      <Select v-model="layout.justify" :options="justifyMap"></Select>
    </div>
    <div class="item">
      <div class="label">Cross Align</div>
...
.layout-group {
  ...
  .align-preview {
    ...
    width: 180px;
    height: 180px;
    border: 1px dashed rgba($panel-light, .5);
    padding: 4px;
    margin-bottom: 10px;
