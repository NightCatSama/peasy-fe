    ...
    ables: [{
      ...
      render: (m, renderer) => {
        ...
        return renderer.createElement('div', {
          ...
          name ? renderer.createElement('div', {
            class: 'moveable-outline-text right-top',
            style: {}
          }, name) : null,
          isResizing ? renderer.createElement('div', {
            class: 'moveable-outline-text left-bottom',
            style: {}
          }, `${
              elem.dataset.resizeWidth || (elem.offsetWidth + 'px')
            } x ${
              elem.dataset.resizeHeight || (elem.offsetHeight + 'px')
            }`) : null,
          isDragging && elem.dataset.leftMove ? renderer.createElement('div', {
            class: 'moveable-outline-line left',
            style: {
              width: `${parseFloat(elem.style.left) * zoom}px`,
            }
          }, renderer.createElement('div', {}, elem.dataset.leftMove)) : null,
          isDragging && elem.dataset.rightMove ? renderer.createElement('div', {
            class: 'moveable-outline-line right',
            style: {
              width: `${parseFloat(elem.style.right) * zoom}px`,
            }
          }, renderer.createElement('div', {}, elem.dataset.rightMove)) : null,
          isDragging && elem.dataset.topMove ? renderer.createElement('div', {
            class: 'moveable-outline-line top',
            style: {
              height: `${parseFloat(elem.style.top) * zoom}px`,
            }
          }, renderer.createElement('div', {}, elem.dataset.topMove)) : null,
          isDragging && elem.dataset.bottomMove ? renderer.createElement('div', {
            class: 'moveable-outline-line bottom',
            style: {
              height: `${parseFloat(elem.style.bottom) * zoom}px`,
            }
          }, renderer.createElement('div', {}, elem.dataset.bottomMove)) : null,
        )
      },
    }],
    props: {
...
export const useMoveable = (elem: HTMLDivElement, item: PageNode, parent?: PageNode) => {
  ...
  /** 记录原先的单位 */
  let units: { width?: string; height?: string; widthReferSize?: number; heightReferSize?: number } = {}
...
export const openDragMode = (activeElem?: HTMLDivElement) => {
  ...
  moveable.on('dragStart', (e) => {
    units = {
      widthReferSize: parent && elem.parentElement
        ? elem.parentElement.clientWidth
        : useDisplayStore().device.width,
      heightReferSize: parent && elem.parentElement
      ? elem.parentElement.clientHeight
      : useDisplayStore().device.height
    }
    isDragging = true
...
export const setSnappableGuidelines = (activeElement?: HTMLDivElement) => {
  ...
  const siblingElement = Array.from(parentElement?.children || []).filter(elem => elem !== activeElem) as HTMLDivElement[]
  ...
  if (referElemList.length > 0) {
    referElemList.forEach(elem => {
      ...
      verticalGuidelines.push(
        wrapperOffset.left,
        (elem.offsetWidth * zoom) + wrapperOffset.left
      )
      horizontalGuidelines.push(
        wrapperOffset.top,
        (elem.offsetHeight * zoom) + wrapperOffset.top
      )
    })
  }
